...
class MAS():
    ...
    def forward(self, taskInfo):
        """
        A multi-agent system for solving mathematics problem.
        Steps:
            1. A planner to provide step-by-step plan.
            2. For each step, either a engineer or mathematician is called to finish the step.
                The engineer generates code and provide the output.
                The mathematician focus on providing formulation and theoretical analyses.
        ...
        for step in steps:
            ...
            if step["role"] == "Engineer":
                ...
                # If the role is Mathematician, generate output based on the mathematician's prompt
                mathematician_prompt = (
                    f"Problem:\n{taskInfo}\n\n"
                    f"Existing insights:\n{existing_insights}\n\n"
                    f"Current step:\n{current_step}\n\n"
                    "A solving plan has been provided for the problem. "
                    "As a Mathematician, your task is to focus on the current step and address the mathematics problem.\n"
                    ...
                    "You should:\n"
                    "1. Defining the mathematical domain or space.\n"
                    "2. Specifying the initial and boundary conditions.\n"
                    "3. Establishing the governing equations and identifying relevant parameters or constants.\n"
                    "4. Formulating the mathematics problem clearly.\n\n"
                )
                # Call llm to generate a response
                output = self.llm.call_llm(mathematician_prompt)
    ...
    def get_plan_steps(self, taskInfo, temperature=None, max_retries=3):
        ...
        If all retry attempts fail, a default set of steps is returned, assigning the given task to both an Engineer and a Mathematician.
        ...
        prompt = (
            ...
            "Each step must be assigned explicitly to either an engineer or a mathematician based on their roles:\n\n"
            "- The Engineer writes code and generates outputs.\n"
            "- The Mathematician defines the mathematical structure, assumptions, initial and boundary conditions, and formulates the mathematical problem.\n\n"
            "Your plan must:\n"
            "1. Break down the problem into clear, executable steps.\n"
            "2. Explicitly assign each step to either the Engineer or the Mathematician.\n"
            "3. Ensure the plan is concise and complete to address all aspects of the problem.\n\n"
            "Output format:\n"
            "Step 1: the description of step1 (Mathematician)\n"
            "Step 2: the description of step2 (Engineer)\n"
        )
        
        attempts = 0  # Track the number of attempts
        step_pattern = r"Step\s+\d+:\s+(.*?)\s+\((Mathematician|Engineer)\)"     # The expected output format pattern
