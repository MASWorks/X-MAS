...
class MAS():
    ...
    def forward(self, taskInfo):
        ...
        Steps:
            1. 5 agents solve the task by writing the code independently.
            2. A final decision-making agent reasons over the solutions and provides the final code.
        """

        # Step-by-step instruction for each chain-of-thought agent to reason and generate code
        cot_instruction = f"Task: {taskInfo}\n\nPlease think step by step and then solve the task by writing the code."

        # Set the number of code solutions to generate; using 5 for variety and diversity
        N = 5
        # Call the llm to generate each solution independently
        cot_results = [self.llm.call_llm(cot_instruction) for _ in range(N)]

        # Use a final decision-making agent to reason over the solutions and provide the final code
    ...
    def get_final_solution(self, taskInfo, solutions):
        ...
        instruction += "Given all the above solutions, reason over them carefully and provide a final answer to solve the task by writing the code."
